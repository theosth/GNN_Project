...
from simulations.elastic_collisions_rust_connector import (
    SimulationData,
    load_rust_simulation_data_from_json,
)
import torch
...
def plot_timestep(ax, positions, velocities, space_size, max_radius):
    ...
    normalized_velocities = (
        velocities / np.linalg.norm(velocities, axis=1, keepdims=True) * space_size
    )
    ...
    for pos in positions:
        circle = patches.Circle(
            (pos[0], pos[1]), radius=max_radius, alpha=0.5, edgecolor="none"
        )
        ax.add_patch(circle)
...
def run_simulation(
    ...
    )  # the only hidden variable that we investigate in this scenario

    simulation = ElasticCollisionSimulation(
        variables=VARIABLES, enable_logging=False, noise=False
    )
...
def get_plot_format_from_python_simulation_json(json_filepath: str):
    ...
    (
        ...
        space_size,
        max_radius,
    ) = get_plot_format_from_python_simulation(simulation)
    return (
        position_history_by_timestep,
        velocity_history_by_timestep,
        space_size,
...
def get_plot_format_from_rust_simulation(simulation_data: SimulationData):
    ...
    for timestep in simulation_data.state_history[
        0
    ]:  # for now only look at one simulation (multiple simulations would be in state_history[0] and state_history[1] etc.)
        ...
        for i in range(simulation_data.num_bodies):
            timestep_positions.append(
                [timestep.positions_x[i], timestep.positions_y[i]]
            )
            timestep_velocities.append(
                [timestep.velocities_x[i], timestep.velocities_y[i]]
            )
        ...
        max_radius = max(
            timestep_radii[0]
        )  # all radii are the same for all bodies currently ! change if needed
...
def get_plot_format_from_rust_simulation_json(json_filepath: str):
    ...
    (
        ...
        space_size,
        max_radius,
    ) = get_plot_format_from_rust_simulation(simulation_data)
    return (
        position_history_by_timestep,
        velocity_history_by_timestep,
        space_size,
